...
public class PlayerVelocity : MonoBehaviour {
    ...
    void HandleWallSliding() {
        wallDirX = (movement.collisionDirection.left) ? -1 : 1;
        bool horizontalCollision = movement.collisionDirection.left || movement.collisionDirection.right;

        if (horizontalCollision && !movement.collisionDirection.below && !movement.forceFall && movement.collisionAngle.onWall) {
            wallContact = true;
    ...
    public void OnJumpInputDown() {
        ...
        if (movement.collisionDirection.below) {
            if (movement.slidingDownMaxSlope) {
                // Jumping away from max slope dir
                if (directionalInput.x != -Mathf.Sign(movement.collisionAngle.slopeNormal.x)) {
                    velocity.y = maxJumpVelocity * movement.collisionAngle.slopeNormal.y;
                    velocity.x = maxJumpVelocity * movement.collisionAngle.slopeNormal.x;
                }
            }
            else {
